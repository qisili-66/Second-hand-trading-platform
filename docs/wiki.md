...
- `POST /api/auth/login`：普通用户登录，账号支持学号、邮箱、昵称或实名，返回 USER JWT；本地演示凭据请以数据库种子脚本为准。
- `POST /api/auth/admin/login`：管理员登录，返回 ADMIN JWT；本地凭据请以数据库种子脚本和本地配置为准。
...
- 本地演示账号分为管理员和普通用户两类；具体凭据请以本地数据库种子脚本为准。管理员只进入数据后台，普通用户只进入用户端交易功能。前端路由阻止普通用户进入 `/admin`，后端后台接口仍以 ADMIN JWT 二次兜底。
...
- 普通用户：校园二手交易买家/卖家。
- 管理员：后台管理账号，只用于数据后台；生产环境必须设置独立密码。
